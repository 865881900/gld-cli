...
export default class WriteFileExpanding extends WriteFile {
  ...
  beganWrite(apiDataMap: Map<string, IApiDataMap>): void {
    ...
    const moduleAxiosPath = path.relative(path.join(process.cwd(), 'src/http/index.js'), path.join(process.cwd(), this.writerDirPath, 'index.js'));
    ...
    // 开始后写入
    this.action(path.join(this.writerDirPath, 'index.js'), moduleStr);
  }
